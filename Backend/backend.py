...
def apply_auto_framing(frame, pose_results):
    ...
    if getattr(pose_results, "pose_landmarks", None):
        ...
        if meas:
            ...
            # Guard against transient invalid Kalman predictions.
...
def start_analysis():
    """Mark analysis as active so frontend start flow succeeds."""
    global camera_active, current_session_data

    with camera_lock:
        camera_active = True

    if not current_session_data.get("start_time"):
        session_start_time = time.time()
        current_session_data["start_time"] = session_start_time
        current_session_data["session_id"] = f"session_{int(session_start_time)}"
        current_session_data["scores"] = []
        current_session_data["feedback_history"] = []
        current_session_data["blink_rate"] = 0
...
def tracked_time():
    ...
    return jsonify({
        ...
        "current_session_time_seconds": round(session_secs, 3),
        "current_session_time_minutes": round(session_secs / 60.0, 3),
